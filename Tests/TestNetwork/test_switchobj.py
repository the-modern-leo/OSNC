...
def test_cdp_neigbhor_interface_single_thread(test):
    """
    For running a a batch update of all the interface description that need to be updated with their cdp neigbhor details
    """
    try:
        s = Stack(test)
        s.login()
    except Exception as e:
        interface_list = [("Ip address", test), ("Interface",), ("Status", "Failed"),
                          ("Error Message", "No Active Connection"), ("config",)]
    ...
    try:
        ...
        interface_list = [("Ip address", test), ("Interface",""), ("Status", "Failed"),
                          ("Error Message", "Unable to gather Information"), ("config","")]
        return interface_list
    interface_list = s.label_cdp_neighbor_interfaces()
    try:
        s.logout()
    except Exception as e:
        return interface_list
...
class test_cdp_neigbhor_interface(unittest.TestCase):

    def test_cdp_neigbhor_interface_tp(self):
        """
        For running a a batch update of all the interface description that need to be updated with their cdp neigbhor details
        """
        try:
            ...
            with ThreadPoolExecutor(max_workers=20) as executor:
                for test in test_batch:
                    ...
                    future_results[executor.submit(test_cdp_neigbhor_interface_single_thread,test)] = test
            ...
            for future in as_completed(future_results):
                ...
                try:
                    ...
                    for interface in item:
                        excel_write.append(interface)
                except Exception as e:
                    raise

            for (item,row) in zip(excel_write,ws.iter_rows(max_col=6, max_row=len(excel_write)+1)):
                try:
                    row[0].value = item['Ip address']
                    row[1].value = item['Hostname']
                    row[2].value = str(item['Interface'])
                    row[3].value = item['Status']
                    row[4].value = item['Error Message']
                    if item['config']:
                        row[5].value = item['config'][1]
                except Exception as e:
                    print(str(e))
                    continue
            wb.save(r'/opt/project/Tests/TestNetwork/key_interface_report.xlsx')
        except Exception as e:
            print(str(e))
            pass
